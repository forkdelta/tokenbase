...
KNOWN_LINK_TYPES = frozenset(
    ('Bitcointalk', 'Blog', 'Chat', 'CoinMarketCap', 'CoinGecko', 'Discord',
     'Email', 'Facebook', 'Github', 'Instagram', 'Linkedin', 'Reddit', 'Slack',
     'Telegram', 'Twitter', 'WeChat', 'Website', 'Whitepaper', 'YouTube'))
...
def test_filename_match_contract(filename, content):
    ...
    assert filename.startswith(
        "tokens/"), "expected file to be in tokens folder"
    assert basename(filename).startswith(
        content["addr"]), "expected filename to match contract address"
...
def test_decimals_equals_erc20_decimals(content):
    ...
    try:
        contract_decimals = web3.eth.contract(
            content["addr"], abi=ERC20_ABI).call().decimals()
...
def test_contract_erc20_totalSupply(content):
    ...
    total_supply = web3.eth.contract(
        content["addr"], abi=ERC20_ABI).call().totalSupply()
...
def test_contract_erc20_balanceOf(content):
    ...
    balance_of = web3.eth.contract(
        content["addr"], abi=ERC20_ABI).call().balanceOf(TEST_BALANCE_ADDR)
...
def test_description_nonempty(content):
    ...
    if len(content["description"]) == 0:
        raise TestWarning(
            "expected description to be non-empty, but got length 0")
...
def test_link_key_known(content, link=None):
    ...
    if key not in KNOWN_LINK_TYPES:
        raise TestWarning(
            "can '{}: {}' be better described by one of well-known types: {}?".
            format(key, value, KNOWN_LINK_TYPES))
...
def test_link_value_https_preferred(content, link=None):
    ...
    if is_valid_uri(value, require_scheme=True):
        ...
        if parsed_value.scheme == "http":
            raise TestWarning(
                "URL scheme is HTTP, but HTTPS is strongly preferred: {}".
                format(value))
...
def test_http_link_active(content, link=None):
    ...
    try:
        r = cfscrape.create_scraper().get(
            value, timeout=30.0, headers={"User-Agent": USER_AGENT})
...
FILE_TESTS = (test_file_extension_yaml, test_filename_match_contract)
CONTENT_TESTS = (test_addr_key_exists, test_addr_0x_string,
                 test_addr_ethereum_address, test_addr_lowercase,
                 ...
                 test_description_safe_html_only, test_links_sequence)
PER_LINK_TESTS = (test_link_key_value_pair, test_link_key_known,
                  test_link_value_uri, test_link_value_https_preferred,
...
def main(targets, quiet=False):
    ...
    for target in targets:
        ...
        for test in generate_tests(target, content):
            try:
                ...
                test_results.append((target, test, exc))
                failures_count += 1
                print("  {} (FAILED - {})".format(test.__doc__,
                                                  failures_count))
    ...
    print(
        len(targets), "targets x", len(CONTENT_TESTS), "tests =",
        len(test_results), "runs, of which:", failures_count, "failures,",
        warnings_count, "warnings,", skip_count, "skipped.")

    import traceback
    printable_results = [
        result for result in test_results if result[-1] is not None
    ]
    print("-" * 88)
    prev_target = None
